  ...
  head: () => ({
    meta: [
      { title: "OpportunityOS — See what's changing, decide what to build" },
      {
        name: "description",
        content:
          "OpportunityOS surfaces evidence-backed opportunities — businesses, products, jobs, trends, online income — with every score fully explained. Live signals from real APIs, sample seeds clearly labelled.",
      },
      {
        property: "og:title",
        content: "OpportunityOS — See what's changing, decide what to build",
      },
      {
        property: "og:description",
        content: "Discover evidence-backed opportunities with fully explainable scoring.",
      },
    ],
  }),
...
function DiscoverPage() {
  ...
  const combined = useMemo(() => {
    const live = (liveQuery.data?.opportunities ?? []).map((o) => ({
      opp: o,
    ...
    <AppShell>
        ...
        <div className="px-4 pt-4 pb-3">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-1">
              ...
              {user ? (
                ...
                <Link
                  to="/auth"
                  className="rounded-full bg-primary px-3 py-1 text-[12px] font-medium text-primary-foreground"
                >
                  Sign in
                </Link>
              )}
          ...
          <p className="mt-1 text-[13px] leading-relaxed text-muted-foreground">
            {liveQuery.isLoading
              ...
              : liveQuery.data?.opportunities.length
                ? `${liveQuery.data.opportunities.length} live signals from Hacker News, plus curated sample opportunities.`
          ...
          <div className="mt-4 flex items-center gap-2">
            ...
            <button
              ...
              className={`grid h-10 w-10 shrink-0 place-items-center rounded-xl border ${showFilters ? "border-primary bg-primary-soft text-primary" : "border-border bg-surface text-muted-foreground"}`}
              aria-label="Filters"
            >
      ...
      <div className="mx-auto max-w-2xl px-4 py-4">
        ...
        {filtered.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-border p-10 text-center text-muted-foreground">
            <p className="text-[13px]">
              {feed === "saved"
                ? "You haven't saved anything yet. Tap the bookmark on a card."
        ...
        <p className="mt-6 text-center text-[11px] leading-relaxed text-muted-foreground">
          Signals labelled <span className="text-state-live">Live</span> come from real public APIs.
          Signals labelled <span className="text-state-sample">Sample</span> are curated seeds used
          to demonstrate the scoring pipeline. Every score is deterministic and fully explainable —
          tap any card.
        </p>
      </div>
    </AppShell>
